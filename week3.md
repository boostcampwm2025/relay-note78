...
- 결과 : 결과는 슬랙 채널 #칭찬하기 게시판에 게시했습니다.
- 느낀 점 : AI를 활용해 배경 (큰틀)은 잡히지만, 디테일한 부분은 아직까진 사람의 손을 타야할 것 같다고 느꼈습니다.
- 인사이트 : 이정도면 적당히 이해했겠지? 싶을때, AI는 "3살 짜리 애기다"라는 마인드로 재차 프롬프트를 점검하기! (AI 이미지 생성만 해당)
...
- 결과 :
> **S-I-C-E — 🛠️ 즉흥적 미니멀리스트**
> 구조 설계보다 상황에 따라 유연하게 접근하지만, 기능과 코드 품질의 균형을 잡으려 노력함
> 반복적인 개선을 통해 코드 품질을 높임
> 팀보다는 독립적으로 작업할 때 몰입도가 높음
> 실험적 접근과 깔끔한 마무리를 병행하는 스타일
- 느낀 점 : 성격이 급해서 마무리나 섬세한 부족한 건가라는 생각이 들었고, 질문이 적어서 모든 내용이 나라고 판단하기 힘들지만 전체적으로 코드 작성에 대한 성향을 볼 수 있어서 부족한 점을 고민하게 되는 것 같다.
- 인사이트 : 리팩토링이나 장기간 프로젝트 혹은 장기간 무언가를 계속해서 꾸준히 하는 작업을 해봐야겠다는 생각이 들었다.
