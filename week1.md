...
Q1. (OX)
스택(Stack)은 FIFO(First In First Out) 방식으로 데이터를 처리한다.
...
Q3. (OX)
배열은 크기가 고정되어 있지만, 연결 리스트는 동적으로 크기를 조절할 수 있다.

Q4. (객관식)
다음 중 트리(Tree) 자료구조의 특징으로 옳은 것을 고르세요.
A. 순환(사이클)이 존재한다
B. 루트 노드는 항상 2개의 자식 노드를 가진다
C. 트리는 하나의 루트 노드를 가지며, 사이클이 없다
D. 트리의 모든 노드는 항상 동일한 깊이를 가진다

Q5. (OX)
해시 테이블은 최악의 경우 검색 시간이 O(n)일 수 있다.
```
...
Q1. ❌ 너의 답: X → 정답: O
해설:
스택(Stack)은 LIFO(Last In First Out) 방식이야.
→ 반대로 FIFO는 큐(Queue)의 동작 방식이야.

Q2. ✅ 너의 답: C → 정답: C
해설:
 • A, B, D는 선형 자료구조 (순차적으로 요소가 연결됨)
 • C. 해시 테이블은 비선형 자료구조, 내부적으로 배열 + 해시 함수로 구성됨

Q3. ✅ 너의 답: O → 정답: O
해설:
 • 배열: 고정 크기, 메모리에 연속적으로 배치됨
 • 연결 리스트: 동적 크기, 노드가 포인터로 연결됨

Q4. ✅ 너의 답: C → 정답: C
해설:
 • A. 트리는 사이클이 없음
 • B. 자식 노드는 0개~여러 개 (2개 고정 아님)
 • D. 깊이는 노드마다 다를 수 있음
 • C. 트리는 루트 노드 + 무사이클 방향성 구조가 핵심

Q5. ❌ 너의 답: X → 정답: O
해설:
 • 해시 테이블은 평균적으로 O(1)의 빠른 검색이 가능하지만,
**충돌(collision)**이 많이 발생하면 최악의 경우 O(n) 이 될 수 있어.
(예: 해시 함수가 모든 데이터를 같은 인덱스로 보내는 경우)
```
...
Q1 - 스택 & 큐 차이 (LIFO vs FIFO)
 • 자료구조 시각화 (VisuAlgo)
→ 스택, 큐 애니메이션으로 이해하기 좋음
 • GeeksforGeeks - Stack vs Queue

Q5 - 해시 테이블의 시간 복잡도
 • MIT OpenCourseWare - 해시 테이블 강의
 • Interview Cake - Hash Table worst-case
```
...
6주차 미션 중 일부
``` 
1. 미션 내용 일부와 내가 정리한 제약 조건을 AI에게 주고, 테스트 케이스 작성 요청
2. 아래와 같이 테스트 케이스 목록 정리
3. 테스트 케이스 목록을 테스트 코드로 변환하여 사용
...
- **입력:** 초기화 후 보드 데이터 조회  
- **기대 결과:**  
  - A, B, C행 중 랜덤 위치에 타노스 2개가 연속된 칸에 배치되어 있음  
  - 두 칸 모두 동일 타노스 인스턴스여야 함  

---

## 2. BlackWidow 최소 1개 배치 테스트
- **조건:** 게임 시작 직후  
- **입력:** 초기화 후 보드 데이터 조회  
- **기대 결과:**  
  - BW가 보드 위에 최소 1개 이상 존재함  

---

## 3. DrStrainge 최소 1개 배치 테스트
- **조건:** 게임 시작 직후  
- **입력:** 초기화 후 보드 데이터 조회  
- **기대 결과:**  
  - DS가 보드 위에 최소 1개 이상 존재함  
...
- **입력:** 초기화 후 보드 데이터 조회  
- **기대 결과:**  
  - TR이 보드 위에 최소 1개 이상 존재함  

---

## 5. 캐릭터 최대 배치 개수 테스트 (예: Loki 최대 3개)
- **조건:** 게임 시작 직후  
- **입력:** 보드 데이터 조회  
- **기대 결과:**  
  - LK가 최대 3개 이하로 배치되어 있음  
...
## 7. 공격자 부재 시 공격 불가 메시지
- **조건:** 공격자 위치에 캐릭터가 없음 (예: A1 빈 칸)  
- **입력:** ‘A1->B1’ 입력  
- **기대 결과:**  
  - “해당 위치에 공격자가 없어서 공격을 중단합니다.” 출력  

---

## 8. BlackWidow 이동 가능 테스트
- **조건:** BW가 D1 위치에서 D2 위치로 이동 시도  
- **입력:** ‘D1->D2’  
- **기대 결과:**  
  - 이동 성공  
  - “MISS (이동합니다)” 메시지 출력  
...
## 9. BlackWidow 이동 불가 테스트
- **조건:** BW가 D1 위치에서 D3 위치로 2칸 이동 시도  
- **입력:** ‘D1->D3’  
- **기대 결과:**  
  - 이동 실패 메시지 출력  
...
## 10. 공격 시 대상 HP 감소 및 제거 테스트
- **조건:** 공격 시도 (예: B2->C3)  
- **입력:** ‘B2->C3’  
- **기대 결과:**  
  - 대상 캐릭터 HP가 공격력만큼 감소  
  - 대상 HP가 0 이하일 경우 보드에서 제거  
